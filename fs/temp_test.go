...
func newFSTemp(cfg agro.Config) agro.FSMetadataService {
	t, _ := NewTemp(cfg)
	if v, ok := t.(agro.FSMetadataService); ok {
		return v
	}
	panic("temp does not implement agro.FSMetadataService")
}

func TestGetVolumes(t *testing.T) {
	m := newFSTemp(agro.Config{})
